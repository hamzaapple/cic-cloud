...
const DhikrBanner = () => {
  ...
  // Visibility toggle logic: 30s visible, 60s hidden
  useEffect(() => {
    let timeout: ReturnType<typeof setTimeout>;
    if (isVisible) {
      // Hide after 30 seconds
      timeout = setTimeout(() => setIsVisible(false), 30 * 1000);
    } else {
      // Show after 60 seconds
      timeout = setTimeout(() => setIsVisible(true), 60 * 1000);
    }
  ...
  return (
    <div className="fixed z-40 pointer-events-none 
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            key={index} // changes when dhikr changes, recreating animation
            drag
            dragMomentum={false}
